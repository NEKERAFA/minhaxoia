# Miñaxoia!

A mobile strategy gessing videogame for two player using Godot Engine

[![Download on Itch.io](https://img.shields.io/badge/Itch.io-Download%20now-FF2449?logo=itchdotio&logoColor=white)](https://nekerafa.itch.io)
[![Made in Godot 4.5.1](https://img.shields.io/badge/Godot-4.5.1-blue?logo=godotengine&logoColor=white)](https://godotengine.org)
[![Under GPLv3.0 license](https://img.shields.io/github/license/NEKERAFA/minhaxoia)](LICENSE)
[![Taiga Project](https://img.shields.io/badge/Taiga-brightgreen?logo=data:image/png;base64,)](https://tree.taiga.io/project/nekerafa-hundir-la-flota)

## Table of contents
- [About](#about)
- [Build](#about)
- [License](#license)

## About

The aim of this project is to develop a functional mobile multiplayer videogame for two players. Each player will participate in a series of short battles that make use of the mechanics of the Battleship game.
These games are strategy type gessing games characterised by a naval theme. However, the final game made will have a cozy theme and simple graphics.

During development, open-source software such as Godot, Blender and GIMP will be used. In adition, Agile methodologies such as Scrum will be used for the planning and project follow-up.

### Mainly features

### Get the game

## Build

## License

> GNU General Public License Version 3, 29 June 2007
>
> Copyright (c) 2025 Rafael Alcalde Azpiazu (NEKERAFA)
>
> You may copy, distribute and modify the software as long as you track
> changes/dates in source files. Any modifications to or software including (via
> compiler) GPL-licensed code must also be made available under the GPL along
> with build & install instructions.
>
> See all legal notice in LICENSE file